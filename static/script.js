$(function(){
    ...
    $('#patients').on('change', function(){
      ...
      $.ajax({
        ...
        success: function(comps){
          ...
          comps.forEach(c => sel.append(`<option value="${c.archetype_id}">${c.name}</option>`));
          loadFields();
        }
      });
    });
      ...
      let row = $(`
        <div class="element-row mb-2 d-flex gap-2">
          ...
          <select class="form-select form-select-sm fn-select">
            <option value="">None</option><option>AVG</option><option>SUM</option><option>COUNT</option>
          </select>
    ...
    $('#runQuery').click(function(){
      ...
      $.ajax({
        ...
        type: 'POST',
        contentType: 'application/json',
        data: JSON.stringify({ ehr_ids, archetype_id: arch, elements, filters, limit, offset, sort }),
